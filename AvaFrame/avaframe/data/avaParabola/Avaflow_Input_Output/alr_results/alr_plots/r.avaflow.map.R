...
# Defining arguments
wkdir <- './data/avaParabola/Avaflow_Input_Output/'  #working directory
...
maxhflow <- 3.500161
maxtflow <- 1369136.000000
maxpflow <- 726478.375000
maxvflow <- 23.197676
maxtreach <- 299.221649
maxhtsun <- 0.000000
maxhentr <- 0.096789
maxhdep <- 0.000000
mapprof <- 0
...
if ( mult == 0 ) {
  ...
  for ( ntimesteps in ntimemin:( ntimemax+1 )) { # loop over all time steps plus one for maps of maximum values
    ...
    if ( mult != 0 ) {  # strings for names of maps:
        ...
        mstringt <- paste(prefix, 'htsun_', fill, sep='')
